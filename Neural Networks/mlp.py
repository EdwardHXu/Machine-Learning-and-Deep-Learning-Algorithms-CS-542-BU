...
class TwoLayerMLP(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    if self.activation is 'relu':
        hidden, cache_hidden_layer = affine_relu_forward(X, W1, b1)
        scores, cache_scores = affine_forward(hidden, W2, b2)
    elif self.activation is 'sigmoid':
        hidden, cache_hidden_layer = affine_sig_forward(X, W1, b1)
        scores, cache_scores = affine_forward(hidden, W2, b2)
    else:
    ...
    # output layer
    data_loss,dscores = softmax_loss(scores,y)  # [PLEASE IMPLEMENT] partial derivative of loss wrt. the logits (dL/dz)
    dW2 = np.dot(hidden.T, dscores)/N  # partial derivative of loss wrt. W2
    db2 = np.mean(dscores, axis=0)     # partial derivation of loss wrt. b2
