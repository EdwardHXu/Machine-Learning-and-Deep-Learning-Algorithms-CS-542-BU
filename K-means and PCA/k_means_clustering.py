...
def get_centroids(samples, clusters):
    ...
    for j in range(K):
        centroid_examples = np.nonzero(clusters == j)[0]
        centroids[j,:] = np.mean( samples[centroid_examples,:], axis=0 )  
    return centroids
...
def find_closest_centroids(samples, centroids):
    ...
    """
    clusters = np.zeros((len(samples),1))
    for i in range(len(samples)):
        point = samples[i]
        mindist = 100
        for j in range(centroids.shape[0]):
            centroid_i = centroids[j]
            distsquared = np.sum(np.square(point-centroid_i))            
...
def choose_random_centroids(samples, K):
    ...
    :return: an array of centroids.
    """
    centroids = samples[np.random.choice(samples.shape[0], K)]
    return centroids
